...
const frida_server = async () => {
	...
	try {
		...
		scriptContent = (await promises.readFile(absoluteScriptPath)).toString();
	...
	// 附加到所有找到的进程并加载脚本
	const sessionsAndScripts: Array<{
		session: frida.Session;
		script: frida.Script;
		pid: number;
	}> = [];
	...
	console.log(
		`[frida] Successfully attached to ${sessionsAndScripts.length} WeChatAppEx processes`,
	);
...
const shutdown = async () => {
	...
	if (fridaSessions.length > 0) {
		console.log(
			`[shutdown] Detaching ${fridaSessions.length} Frida sessions...`,
		);
	...
	if (debugWSS) {
		...
		closePromises.push(
			new Promise<void>((resolve, reject) => {
				debugWSS!.close((err) => {
					if (err) {
						console.error("[shutdown] Error closing debug WSS:", err);
					} else {
						console.log("[shutdown] Debug WebSocket server closed.");
					}
	...
	if (proxyWSS) {
		...
		closePromises.push(
			new Promise<void>((resolve, reject) => {
				proxyWSS!.close((err) => {
					if (err) {
						console.error("[shutdown] Error closing proxy WSS:", err);
					} else {
						console.log("[shutdown] Proxy WebSocket server closed.");
					}
					resolve();
				});
			}),
